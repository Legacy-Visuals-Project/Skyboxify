    ...
    public static StaticGeometry create(final VertexFormat vertexFormat, final VertexFormat.Mode vertexMode, final int vertexCount, final Consumer<VertexConsumer> vertexConsumer) {
        try (final ByteBufferBuilder byteBufferBuilder = new ByteBufferBuilder(vertexFormat.getVertexSize() * vertexCount)) {
            ...
            try (final MeshData meshData = builder.buildOrThrow()) {
                ...
                final GpuBuffer vertexBuffer = device.createBuffer(() -> "Static geometry vertex buffer", BufferType.VERTICES, BufferUsage.STATIC_WRITE, meshData.vertexBuffer());
                ...
                if (indexByteBuffer == null) {
                    ...
                    indexBuffer = device.createBuffer(() -> "Static geometry auto index buffer", BufferType.INDICES, BufferUsage.STATIC_WRITE, autoIndexBuffer.size());

                    final CommandEncoder commandEncoder = device.createCommandEncoder();
                    try (final GpuBuffer.ReadView readView = commandEncoder.readBuffer(autoIndexBuffer)) {
                        commandEncoder.writeToBuffer(indexBuffer, readView.data(), indexBuffer.size);
                    }

                    indexType = autoStorageIndexBuffer.type();
                } else {
                    indexBuffer = device.createBuffer(() -> "Static geometry index buffer", BufferType.INDICES, BufferUsage.STATIC_WRITE, indexByteBuffer);
