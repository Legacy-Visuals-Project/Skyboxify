...
public final class SkyboxParser {
    ...
    public static @Nullable JsonObject parseSkyProperties(final Properties properties, final Id propertiesIdentifier, final ResourcePack packResources) {
        final JsonObject output = new JsonObject();

        final Id sourceTexturePath = parseSourceTexture(properties, propertiesIdentifier, packResources);
    ...
    private static Id parseSourceTexture(final Properties properties, final Id propertiesIdentifier, final ResourcePack packResources) {
        ...
        final DataResult<Id> sourceResult = ParserCodecs.getSourceTextureCodec(propertiesIdentifier).parse(JavaOps.INSTANCE, source);
        if (sourceResult.isError()) {
            LOGGER.error("{}", sourceResult.error().get());
            return null;
        }

        final Id sourceTextureIdentifier = sourceResult.getOrThrow();
        try {
            if (!SkyboxifyImpl.config().debug.isEnabled()) {
                final IOSupplier<InputStream> sourceTextureStream = packResources.getResource(ResourceType.CLIENT_ASSETS, sourceTextureIdentifier);
